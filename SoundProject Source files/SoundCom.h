...
{
public:
  SoundCom(wstring sOutputDevice, unsigned int nSampleRate = 44100, unsigned int nChannels = 1, unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
  {
...
public:
  ...
  void SetLeftFunction(double(*func)(double))
  {
    m_leftFunction = func;
  }
  
  void SetRightFunction(double(*func)(double))
  {
    m_rightFunction = func;
  }
...
private:
  double(*m_leftFunction)(double);
  double(*m_rightFunction)(double);
  ...
  void MainThread()
  {
    ...
    while (m_bReady)
    {
      ...
      for (unsigned int n = 0; n < m_nBlockSamples; n+=2)
      {
        // User Process
        if (m_leftFunction == nullptr || m_rightFunction == nullptr)
        {
          nLeftNewSample = (T)(clip(UserProcess(m_dGlobalTime), 1.0) * dMaxSample);
          nRightNewSample = (T)(clip(UserProcess(m_dGlobalTime), 1.0) * dMaxSample);
        }
        else
        {
          nLeftNewSample = (T)(clip(m_leftFunction(m_dGlobalTime), 1.0) * dMaxSample);
          nRightNewSample = (T)(clip(m_rightFunction(m_dGlobalTime), 1.0) * dMaxSample);
        }

        m_pBlockMemory[nCurrentBlock + n] = nLeftNewSample;
        m_pBlockMemory[nCurrentBlock + n + 1] = nRightNewSample;
        nPreviousSample = nLeftNewSample;
